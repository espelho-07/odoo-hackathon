...
import { getCurrentUser } from '../utils/storage'; // Auth user only
...
const TimeOff = () => {
    ...
    const fetchLeaves = async () => {
        setLoading(true);
        try {
            const response = await api.getLeaves(isAdmin ? null : user.id, null);
    ...
    const columns = [
        ...
        { header: "Dates", accessor: (row) => `${row.start_date.split('T')[0]} to ${row.end_date.split('T')[0]}` },
    ...
    if (isAdmin) {
        columns.unshift({ header: "Employee", accessor: "name" });
    }
